-- ============================================================================
-- Sin este SQL, el sistema RAG que armamos en Python NO PUEDE FUNCIONAR:
--
-- 1. `vector_store.search()` / `repository.search()` llaman al RPC
--    "match_knowledge" — si no existe esta función en Postgres, cualquier
--    búsqueda revienta con "function match_knowledge does not exist".
--
...
--
-- Ajusta `vector(1536)` según la dimensión real con la que creaste la
-- columna `knowledge_chunks.embedding` (1536 = OpenAI text-embedding-3-small,
-- el proveedor por defecto en config.py). Si usas Voyage con
-- output_dimension distinto, cambia el número aquí también.
-- ============================================================================
