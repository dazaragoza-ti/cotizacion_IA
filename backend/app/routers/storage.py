...
async def optimize_storage_file(
    ...
    """Descarga un .glb desde Storage, lo optimiza con glTF-Transform y lo vuelve a subir.

    Usa el comando `optimize` (Draco para geometría + WebP para texturas), no solo `draco`,
    porque este último no comprime texturas y dejaba la reducción total en casi nada.
    Requiere `node` en el servidor y @gltf-transform/cli instalado en backend/node_modules.
    """
    service_client = supabase_service if supabase_service else supabase
    try:
        ...
        try:
            ...
            # 3) Ejecutar gltf-transform "optimize" via el JS entry point directamente con node.
            # Se invoca el .js con node (no el .cmd/.sh) para evitar problemas de permisos y de
            # ejecutabilidad tanto en Windows como en Linux.
            # A diferencia del comando "draco" (que SOLO comprime geometría), "optimize" corre el
            # pipeline completo e incluye la compresión de texturas —que suelen ser la mayor parte
            # del peso de un .glb—, por eso con "draco" a secas la reducción era mínima.
            ...
            # `encoder_method` se conserva por compatibilidad con el frontend, pero `optimize`
            # no expone --method; usa los parámetros de cuantización por defecto de Draco.
            _ = encoder_method
            cmd = [
                'node', cli_js, 'optimize', tmp_in.name, tmp_out.name,
                '--compress', 'draco',          # compresión de geometría con Draco
                '--texture-compress', 'webp',   # recomprime texturas a WebP (el mayor ahorro real)
                '--texture-size', '2048',       # limita las dimensiones máximas de textura
                '--simplify', 'false',          # NO reducir triángulos (mantiene la geometría exacta)
                '--join', 'false',              # NO fusionar mallas (conserva piezas/nodos del visor)
                '--flatten', 'false',           # NO aplanar el grafo de escena
            ]
            try:
            ...
            # Salvaguarda: en casos patológicos (texturas ya comprimidas, modelos muy
            # pequeños) el resultado puede ser IGUAL o MAYOR que el original. En ese caso
            # conservamos el original y no lo reemplazamos por una versión más pesada.
            if len(compressed_bytes) >= len(original_bytes):
                print(
                    f"[optimize] sin ganancia: original={len(original_bytes)} "
                    f"optimizado={len(compressed_bytes)}; se conserva el original."
                )
                return {
                    "status": "skipped",
                    "bucket": bucket,
                    ...
                    "message": "El archivo ya estaba optimizado; no se logró reducir su tamaño.",
                }

            # 5) Subir el archivo optimizado reemplazando el original
            service_client.storage.from_(bucket).upload(
                path=path,
